...
<div class="section world-news">
	<div class="row">
		<div class="col-md-8 col-sm-6">
			...
			<?php
			$post = Post::where('post_category','=',$categories[0]->id)->orderByRaw("RAND()")->first();
			if($post){
			...
			$r_posts = Post::orderByRaw("RAND()")->take(8)->get();
			?>
			...
			<div class="post large-post">
				...
				<div class="list-post">
					<ul>
						...
						<?php
						$title = mb_substr($post->title,0, 55) . " ...";
						?>
						<li><a href="/blog/{{$post->slug}}" title="{{$post->title}}">{{$title}} <i class="fa fa-angle-right"></i></a></li>
						@endforeach
					</ul>
				</div>
			</div>
		...
		<div class="col-md-4 col-sm-6">
			<div class="middle-content">
				...
				@include('blog.partials.3slidershort')
			</div>
		</div>
	</div>
</div>
